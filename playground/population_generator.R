...
generate_humans <- function(t, start_id, number, start_age = NA) {
  tibble::tibble(
    ...
    current_age = ifelse(is.na(start_age), as.integer(runif(number, min = 0, max = death_age)), start_age),
    dead = FALSE,
...
for (t in 1:100) {
  humans[!humans$dead, ] %<>%
    dplyr::mutate(
      ...
      death_time = ifelse(dead, t, NA_integer_)
    )
  ...
  if (deaths > 0) {
    humans %<>% rbind(
      generate_humans(t, max(humans$id + 1), deaths, 0)
    )
  }

}

nrow(humans[!humans$dead,])
